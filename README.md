...
Foldocs uses [Tegami](https://tegami.fuma-nama.dev) for changelogs,
versioning, release pull requests, and publishing. Create a release note with
`pnpm tegami`, commit the generated `.tegami/*.md` file, and merge it into
`main`. The `publish.yml` workflow opens or updates the **Version Packages**
pull request. Merging that pull request publishes the packages and creates the
GitHub releases.

Only the 13 packages required by the framework, generated application, and
their runtime dependency graph are published. Optional providers and generators
remain private until their public APIs are ready.

npm publishing uses GitHub Actions trusted publishing (OIDC). There is no
`NPM_TOKEN` or `NODE_AUTH_TOKEN` GitHub secret. To reserve new package names and
connect them to `.github/workflows/publish.yml`, authenticate locally with npm
and run fledgling:

```bash
npm login
pnpm npm:setup:dry-run
pnpm npm:setup
```

Run `pnpm npm:trust` later to reconcile trusted-publisher settings for existing
packages. The npm configuration must identify `Aniket-508/foldocs` and the
workflow filename `publish.yml`, with no GitHub environment restriction.

## Quality checks

The repository follows Foldkit's quality pipeline. Run the complete local gate
...
The command verifies formatting, Oxlint rules, unused files and dependencies,
circular imports, repository scripts, the npm release boundary, package builds,
TypeScript, and tests. `pnpm lint:fix` and `pnpm format` apply safe automatic
fixes. A pre-push hook runs the complete gate plus the Playwright smoke suite.

Applications generated by `create-foldocs` include the same Prettier import
sorting, Foldkit Oxlint preset, formatting, linting, type checking, tests, and
documentation validation through their own `check` script.
