...
describe('documentation layout', () => {
  it('distinguishes section headings, folder dropdowns, and page context', () => {
    ...
    expect(Option.isSome(Scene.find(rendered, '.fd-sidebar-folder'))).toBe(true)
    expect(
      Option.isSome(Scene.find(rendered, '.fd-sidebar-folder-active')),
    ).toBe(false)
    expect(
    ...
    expect(Option.isSome(Scene.find(rendered, '.fd-sidebar-chevron'))).toBe(
      true,
    )
