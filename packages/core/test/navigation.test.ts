...
describe('navigation', () => {
  ...
  it('hides unlisted pages unless meta.json contains an ellipsis', () => {
    const listed = page('listed', 'Listed')
    const hiddenFromTree = page('legacy', 'Legacy')

    expect(
      buildNavigation([listed, hiddenFromTree], {
        '': { pages: ['listed'] },
      }).map(node => node.label),
    ).toEqual(['Listed'])
    expect(
